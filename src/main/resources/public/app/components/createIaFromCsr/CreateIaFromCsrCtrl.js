mainApp
		.controller(
				'CreateIaFromCsrCtrl',
				function($state, $http, $window, certificateTree, caUtils) {
					...
					self.onKeyInjectionSuccess = function() {
						$state.go('createIAFromCsr.basics');
					};

					self.onBasicsSubmit = function() {
						if (self.formData.basics.$valid) {
							$state.go('createIAFromCsr.extensions');
						} else {
							$window
					...
					self.processForm = function() {
						...
						caUtils.createIAFromCsr(data, function() {
							$state.go('listAllCerts');
						});
